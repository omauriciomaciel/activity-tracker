...
* chore(ci): atualiza python-semantic-release e refatora changelog

- Atualiza a versão do python-semantic-release para 9.12.0
- Reestrutura o CHANGELOG.md para um formato mais conciso
- Adiciona padrões de exclusão de commit no pyproject.toml
- Evita que merges e versões sejam listadas redundantemente no log ([`170b7f1`](https://github.com/omauriciomaciel/activity-tracker/commit/170b7f1f1dd6eafc7600e683aae97957b93d54b2))

* chore(install): torna o script de instalação POSIX compatível

- Altera o shebang de bash para sh para maior portabilidade
- Substitui sintaxes específicas do Bash por comandos POSIX
- Troca echo -e por printf para garantir consistência de cores
- Remove dependências de bash-isms em testes de arquivos e strings ([`e12aa61`](https://github.com/omauriciomaciel/activity-tracker/commit/e12aa61a04b2b2b9cd9b27fca766157995d8f426))

### Documentation

* docs(readme): documenta nova aba de configurações na TUI

- Adiciona descrição e funcionamento da aba Config
- Atualiza a tabela de atalhos com novos comandos de navegação e edição
- Inclui mapeamentos de teclas para manipulação de privacidade e recarga
- Alinha a representação visual da interface com a implementação atual ([`c39e66f`](https://github.com/omauriciomaciel/activity-tracker/commit/c39e66f46bc45e19904dd1362d12f3a853a8b30d))

### Features

* feat(notion): melhora conversão de markdown para blocos

- Implementa suporte a blocos de código com detecção de linguagem
- Adiciona suporte a cabeçalhos ATX (#, ##, ###)
- Suporta itens de lista com recuo (sub-bullets)
- Aprimora o parse de textos inline para código, negrito e itálico ([`f2263c9`](https://github.com/omauriciomaciel/activity-tracker/commit/f2263c9f9b722a50cf21c658f42a5fa5457cb7f1))

* feat(tui): adiciona renderização básica de markdown

- Implementa suporte a negrito, cabeçalhos e listas aninhadas
- Substitui a renderização de texto simples por linhas estilizadas
- Melhora a visualização de resumos gerados por LLM no terminal
- Adiciona parser para converter sintaxe markdown em Spans do Ratatui ([`c4a90a1`](https://github.com/omauriciomaciel/activity-tracker/commit/c4a90a1b150742eb25e21a5d7f9660481acbea9c))

* feat(tui): agrupa abas duplicadas na renderização de atividades

- Implementa deduplicação de abas para evitar repetições visuais
- Adiciona contador [Nx] para indicar múltiplas abas do mesmo site
- Mantém a ordem de primeira ocorrência dos links renderizados
- Melhora a legibilidade da interface quando há muitos sites abertos ([`093466d`](https://github.com/omauriciomaciel/activity-tracker/commit/093466d8dcbf2b60af4ef55602d7ca1c0c0085f0))

* feat(collector): adiciona funcionalidade de remover tags

- Implementa função delete_tag para filtrar entradas em arquivos jsonl
- Adiciona flag --delete ao comando de tags no CLI
- Valida a presença do label ao tentar excluir uma tag
- Permite a remoção de tags em datas específicas ou no dia atual ([`a1cc9c0`](https://github.com/omauriciomaciel/activity-tracker/commit/a1cc9c044fa8159f9444b50b55b6e171d8a279c1))

* feat(tui): implementa suporte a interação via mouse

- Adiciona manipulação de eventos de mouse no loop principal
- Permite navegação entre abas e alteração de dias via cliques
- Implementa scroll do mouse para navegação vertical de conteúdo
- Mapeia linhas da tela de configuração para campos de dados
- Permite seleção de campos de configuração através de cliques ([`7dfd836`](https://github.com/omauriciomaciel/activity-tracker/commit/7dfd8360ccf7b96d4e84d3883f96921c555d8c14))

* feat(config): adiciona suporte a prompts customizados para o LLM

- Permite definir e limpar prompts via CLI e TUI
- Implementa placeholders {context} e {lang} no template
- Adiciona renderização multi-linha e cursor avançado na TUI
- Suporta quebras de linha literais (\n) em prompts customizados
- Expande a lista de idiomas suportados no prompt padrão ([`17c2cdc`](https://github.com/omauriciomaciel/activity-tracker/commit/17c2cdcb243a7426be77c25b4c7f2309e4aac783))

* feat(core): adiciona funcionalidade de anotações manuais (tags)

- Implementa comando `at tag` para criar e listar notas diárias
- Adiciona suporte a tags no armazenamento JSONL e no sumarizador
- Integra exibição de notas na interface TUI e no contexto de IA
- Inclui suporte a data específica para inserção e consulta de tags
- Atualiza documentação no README com exemplos de uso ([`b84fad3`](https://github.com/omauriciomaciel/activity-tracker/commit/b84fad314d3fbebdebdcfeda1c87eb604fa82cbd))

### Refactoring

* refactor(summarizer, tui): reestrutura módulos de resumo e interface

- Move a lógica de agregação, exportação e LLM para arquivos separados
- Decompõe o módulo TUI em sub-módulos de renderização, edição e configuração
- Organiza a estrutura de diretórios para melhorar a manutenibilidade
- Remove arquivos monolíticos em favor de uma arquitetura modular ([`1c36687`](https://github.com/omauriciomaciel/activity-tracker/commit/1c36687fe587f36f2893f1d7d2f974fdd5cdc34f))
...
* chore(release): 1.9.0 ([`af03570`](https://github.com/omauriciomaciel/activity-tracker/commit/af03570dfa38f81fa5dd5e1dbdc114dbe3b8f991))
